...
<span class="submenu"><a  id="addworkertype" class="btn button_btn bg-deep-blue" title="添加职位"><i class="fa  fa-edit"></i>&nbsp;添加职位</a></span>
</div>
<!--列表展示-->
<div class="list_Exhibition margin-sx">
 <table class="table table_list table_striped table-bordered" id="sample-table">
  ...
  <tbody>
  <c:forEach items="${worker }" var="work">
    <tr class="main">
    	<c:if test="${work.getStatus()==0 }">
    		<td class="center"><label><input btn_name="checkid" type="checkbox" class="ace" value=""><span class="lbl"></span></label></td>
    	</c:if>
    	<c:if test="${work.getStatus()==1 }">
    		<td class="center"><label><input btn_name="checkid" type="checkbox" class="ace" value="${work.getWorkerTypeId()}"><span class="lbl"></span></label></td>
    	</c:if>
   		<td><c:out value="${work.getWorkerTypeId() }"></c:out></td>
   		<td><c:out value="${work.getWorkerTypeName() }"></c:out></td>
   		<td><c:out value="${num.get(work.getWorkerTypeId()) }"></c:out></td>
   		<td><c:out value="${work.getDiscripe() }"></c:out></td>
   		<td><c:out value="${work.getAddTime() }"></c:out></td>
   		<td>
   			<c:if test="${work.getStatus()==1 }">
	   			<a title="编辑" onclick="Competence_upe(this,${work.getWorkerTypeId() })" href="javascript:;" class="btn button_btn bg-deep-blue">编辑</a>        
	            <a title="删除" href="javascript:;" onclick="Competence_del(this,${work.getWorkerTypeId() })" class="btn button_btn btn-danger">删除</a>
            </c:if>
            <c:if test="${work.getStatus()==0}">
            	<a title="编辑" onclick="Competence_upe(this,${work.getWorkerTypeId() })" href="javascript:;" class="btn button_btn bg-deep-blue">编辑</a>        
            </c:if>
...
$("#prodelete").click(function(){
	layer.confirm('确认要删除吗？',{icon:0,},function(index){
		...
		layer.open({
		    ...
		    ,content: '../deleteworkertype.do?workertypeid='+status,
		});
	});
})
...
function Competence_del(obj,id){
	layer.confirm('确认要删除吗？',{icon:0,},function(index){
		layer.open({
		    ...
		    ,maxmin: true //允许全屏最小化
		    ,anim: 2 //0-6的动画形式，-1不开启
		    ,content: '../deleteworkertype.do?workertypeid='+id,
...
function Competence_upe(obj,id){
	layer.open({
	    ...
	    ,maxmin: true //允许全屏最小化
	    ,anim: 2 //0-6的动画形式，-1不开启
	    ,content: '../updateworkertypeset.do?workertypeid='+id,
