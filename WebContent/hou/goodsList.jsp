...
<div class="list_Exhibition margin-sx">
 <table class="table table_list table_striped table-bordered" id="sample-table">
  ...
  <c:forEach items="${goods }" var="good">
    <tr class="main">
    <c:if test="${good.getStatus()==0||good.getSellStatus()==0 }">
    	...
    	<td class="center"><label><input btn_name="checkid" type="checkbox" class="ace" value=""><span class="lbl"></span></label></td>
   		...
   		<td><c:out value="${good.getGoodsId() }"></c:out></td>
   		<td><c:out value="${good.getGoodsName() }"></c:out></td>
   		<td><img src="${good.getPicture() }" width="100px" height="80px"></td>
   		<td><c:out value="${typename.get(good.getTypeId()) }"></c:out></td>
   		<td><c:out value="${clientname.get(good.getClientId()) }"></c:out></td>
   		<td><c:out value="${good.getPrice() }"></c:out></td>
   		<td><c:out value="${good.getDiscripe() }"></c:out></td>
   			<c:if test="${good.getStatus()==0}">
   			  <td class="td-status"><span class="label label-success label-sm">已下架</span></td>
   			</c:if>
   			<c:if test="${good.getStatus()==1}">
   			  <td class="td-status"><span class="label label-success label-sm">上架中</span></td>
   			</c:if>
   		<td>
   			<c:if test="${good.getStatus()==0}">
   				<a style="text-decoration:none" class="btn button_btn btn-gray" onClick="Competence_start(this,${good.getGoodsId()})" href="javascript:;" title="上架">上架</a>
   			</c:if>
   			<c:if test="${good.getStatus()==1}">
   				<a title="下架" onclick="Competence_close(this,${good.getGoodsId()})" href="javascript:;" class="btn button_btn btn-Dark-success">下架</a> 
   			</c:if>
   			<c:if test="${good.getStatus()==0||good.getSellStatus()==0 }">
	            <a title="删除" href="javascript:;" onclick="Competence_del(this,${good.getGoodsId() })" class="btn button_btn btn-danger">删除</a>
            </c:if>
            <a title="查看" onclick="Competence_view(this,${good.getGoodsId() })" href="javascript:;" class="btn button_btn bg-deep-blue">查看</a>        
   		</td>
   	</tr>
  </c:forEach>
  </tbody>
...
$("#prodelete").click(function(){
	layer.confirm('确认要删除吗？',{icon:0,},function(index){
		...
		layer.open({
		    ...
		    ,content: '../deletegoods.do?goodsid='+status,
		});
	});
})
...
function Competence_del(obj,id){
	layer.confirm('确认要删除吗？',{icon:0,},function(index){
		layer.open({
		    ...
		    ,maxmin: true //允许全屏最小化
		    ,anim: 2 //0-6的动画形式，-1不开启
		    ,content: '../deletegoods.do?goodsid='+id,
...
function Competence_close(obj,id){
	layer.confirm('确认要下架吗？',{icon:0,},function(index){
		layer.open({
		    ...
		    ,maxmin: true //允许全屏最小化
		    ,anim: 2 //0-6的动画形式，-1不开启
		    ,content: '../changegoodsstatus.do?status=0&id='+id,
...
function Competence_start(obj,id){
	layer.confirm('确认要上架吗？',{icon:0,},function(index){
		layer.open({
		    ...
		    ,maxmin: true //允许全屏最小化
		    ,anim: 2 //0-6的动画形式，-1不开启
		    ,content: '../changegoodsstatus.do?status=1&id='+id,
...
function Competence_view(obj,id){
	layer.open({
	    ...
	    ,maxmin: true //允许全屏最小化
	    ,anim: 2 //0-6的动画形式，-1不开启
	    ,content: '../viewgoods.do?goodsid='+id,
