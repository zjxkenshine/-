...
<!---------------------produta-------------------------->
 ...
 <!---------------------individual-------------------------->
	...
	<div class="indiv">
    	...
    	<div class="right indivters"><div>${client.getClientName() }，欢迎您！</div>
    	  <div class="indivters_search">
    	 		<a id="updateclient"><button>修改个人信息</button></a>
    	 		<a id="updateclientpassword"><button>修改密码</button></a>
         	   <ul>
                    <li>真实姓名： ${client.getRealName() }</li>
                    <li>联系电话： ${client.getTel() }</li>
                    <li>联系地址： ${client.getProvince() }${client.getCity() }${client.getSchool() }</li>
                    <li>身份证：${client.getIdcard() }</li>
                    <li>权限：<c:if test="${client.getStatus()==0}" >二级（无法发布商品及兼职信息）</c:if><c:if test="${client.getStatus()==1}" >一级（可发布商品及兼职信息）</c:if></li>
                    <li>信誉积分：${client.getTruhCode() }</li>
                    <li>待处理订单（0）</li>
                    <li></li>
                </ul>
            </div>
